...
- **修复长对话消息列表排版错位** — ：`ConversationManager` 有滑动窗口（超 35 条从头部裁剪最旧消息），而 ViewModel 内存列表从不裁剪。熄屏重开后 `reloadHistory` 用「历史 + 所有不在历史的消息」重建，会把被窗口裁掉的旧消息误判为"新消息"拼到列表末尾 → 顺序错乱。修复：以历史最后一个 id 为分界合并——分界前一律以历史为准（丢弃落后旧副本），分界后才保留真正新追加（乐观消息、触摸气泡），启动预热与 reload 共用同一合并逻辑。
