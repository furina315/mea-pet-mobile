...
- **修复长对话消息列表排版错位** — 两个叠加因素一并修复：
  - **列表对齐**：`LazyColumn` 移除 `Arrangement.Bottom` 底部对齐（内容超视口时与虚拟化 / key 复用叠加会产生气泡串位），滚动到底改由「最新一条消息 id 变化」驱动，触摸气泡从中间移除、历史合并等不再打断阅读位置。
  - **会话裁剪合并**：`ConversationManager` 有滑动窗口（超 35 条从头部裁剪最旧消息），而 ViewModel 内存列表从不裁剪。熄屏重开后 `reloadHistory` 用「历史 + 所有不在历史的消息」重建，会把被窗口裁掉的旧消息误判为"新消息"拼到列表末尾 → 顺序错乱。修复：以历史最后一个 id 为分界合并——分界前一律以历史为准（丢弃落后旧副本），分界后才保留真正新追加（乐观消息、触摸气泡），启动预热与 reload 共用同一合并逻辑。
- **修复喇叭图标时机** — 喇叭「播放中」图标不再在 TTS 合成阶段就亮（消息出来、语音还在生成时误亮），改为 **AudioTrack 真正开始播放前一刻才点亮**，播完（末尾帧标记）即灭。

### Changed

- **TTS 自然度优化** — 合成参数由通用默认改为针对性调优：`noiseScale` 0.667→0.5、`noiseScaleW` 0.8→0.65（音高更稳、节奏更自然）；句末自动补 `#` 韵律停顿符号（训练口径内），解决长句一口气读完的生硬感。
- **语速滑杆重做** — ① 改用与设置页其他滑杆一致的双缓冲拖动：本地状态跟手、松手（`onValueChangeFinished`）才写盘，不再每帧落 DataStore 导致拖动卡顿；② 固定 0.1 粒度步进（0.5–2.0，steps=14）；③ **修正方向**：滑块与显示改为「语速倍率」（0.5=半速慢、1.0=原速、2.0=双倍速快），内部换算为 `length_scale`（`1/speed`），不再出现「显示 0.5x 实际很快」的反直觉问题。
- **TTS 排查日志增强** — `TtsManager` / `TtsAudioPlayer` 全链路关键节点（开关 / 模型态 / 合成 / 播放 / 打断 / 完成）补详细日志，便于后续问题定位。
